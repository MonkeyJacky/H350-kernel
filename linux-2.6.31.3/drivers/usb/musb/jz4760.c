...
#include "musb_core.h"
#define D(msg, fmt...)  \
	printk(KERN_INFO ": %s(): "msg, __func__, ##fmt)

extern int act8600_set_q1(int enable);
extern int act8600_set_q3(int enable);

unsigned int b_as_host_remove;
unsigned int b_start_host;
...
static void otg_id_pin_stable_func(unsigned long data)
{
	...
	unsigned int id;
        ...
        id = __gpio_get_pin(GPIO_OTG_ID_PIN);
        D("id is %d\n",id);

        if(!id)
        { /*host*/
		act8600_set_q1(1);
                b_as_host_remove = 1;   // id low irq, as host , b_as_host_remove = 1
                __gpio_mask_irq(OTG_HOTPLUG_PIN);
                __gpio_as_irq_rise_edge(GPIO_OTG_ID_PIN);
                musb_start(musb);
        }
