...
struct uh_data {
	...
	struct timer_list keep_alive_timer; /* Keep alive */
        ...
        struct timer_list vbus_down_timer;
        unsigned int vbus_down_num;     // id pin high counter
        unsigned int is_vbus_down_timer_start;  // detected id pin high
...
extern unsigned int b_as_host_remove;   //remove device form otg 
extern unsigned int b_start_host;

static inline void otg_vbus_down_start(struct uh_data *uh)
{
        if(uh->is_vbus_down_timer_start)
                return;
        uh->is_vbus_down_timer_start = 1;
        mod_timer(&uh->vbus_down_timer,5+jiffies);
}

static void otg_vbus_down_func(unsigned long data)
{
        struct uh_data *uh = (struct uh_data *)data;
        if(uh->vbus_down_num>3)
        {
                b_as_host_remove = 0;
                uh->is_vbus_down_timer_start = 0;
                uh->vbus_down_num = 0;
	        if(b_start_host==2)
	        {
			b_start_host = 3 ;
	...
	D(">>> otg vbus down timer func : num=%d,start=%d\n",uh->vbus_down_num,uh->is_vbus_down_timer_start);
...
{
	//printk("\n medive printk : vbus hotpul do detect!\n");
	D("Called.\n");
	...
	if(__gpio_get_pin(uh->gpio_pin)) {
		...
		if (uh->cable_detect_state == 1){
			printk("\n medive printk: cable state is  UH_CABLE_STATE_POWER\n");
		}else if (uh->cable_detect_state == 2){
			printk("\n medive printk: cable state is  UH_CABLE_STATE_USB\n");
		}
		set_cable_state(uh, uh->cable_detect_state);
	}else{
		printk("\n medive printk: cable state is  UH_CABLE_STATE_OFFLINE\n");
...
static int uh_setup(struct platform_device *pdev, int gpio_irq, int gpio_pin)
{
	...
	setup_timer(&uh->stable_timer, uh_stable_timer_func, (unsigned long)uh);
        ...
        setup_timer(&uh->vbus_down_timer, otg_vbus_down_func, (unsigned long)uh); // mask host hotplug event
        uh->vbus_down_num = 0;
        uh->is_vbus_down_timer_start = 0;
