...
static int tvout_mode = 0;
static int hdmi_mode = 0;
static unsigned int h350_bklight = 100;
...
//		 .fg0 = {32, 0, 0, 720, 482}, /* bpp, x, y, w, h */
	...
	.panel = {
		...
		.slcd_cfg = 0,
		.ctrl = LCD_CTRL_OFUM | LCD_CTRL_BST_16,	/* 16words burst, enable out FIFO underrun irq */
	//.ctrl = LCD_CTRL_OFUM | LCD_CTRL_BST_64,  //medive change for hdmi gpu	/* 16words burst, enable out FIFO underrun irq */
		//320, 240, 60, 1, 1, 40, 215, 10, 34,
		//320, 240, 50, 1, 1, 686, 70, 12, 11,
		320, 240, 60, 1, 1, 686, 69, 12, 11,
	},
	.osd = {
		 .osd_cfg = LCD_OSDC_OSDEN | /* Use OSD mode */
		 LCD_OSDC_ALPHAEN | /* enable alpha */
		 ...
		 .osd_ctrl = 0,		/* disable ipu,  */
		 .rgb_ctrl = 0,
		 .bgcolor = 0x00, /* set background color Black */
		 .colorkey0 = 0x80000000, /* disable colorkey */
		 .colorkey1 = 0x00, /* disable colorkey */
		 .alpha = 0x80,	/* alpha value */
...
struct jz4760lcd_info jz4760_info_hdmi_576p = {                            
        .panel = {                                                     
                ...
                .slcd_cfg = 0,                                            
	//	.ctrl = LCD_CTRL_BST_32, 
		.ctrl = LCD_CTRL_BST_32|LCD_CTRL_OFUM,   //medive change for hdmi gpu                               
...
struct jz4760lcd_info jz4760_info_hdmi_1080p30 = {
        .panel = {
                ...
                .slcd_cfg = 0,
               // .ctrl = LCD_CTRL_BST_32,
	    .ctrl = LCD_CTRL_BST_64|LCD_CTRL_OFUM,  //medive change for hdmi  gpu
                //  width,height,freq,hsync,vsync,elw,blw,efw,bfw
...
extern void jz4770_i2s_set_external_codec(void);
extern void jz4770_i2s_set_internal_codec(void);
...
static unsigned char *lcd_frame01 = NULL;
...
static void jz4760lcd_info_switch_to_TVE(int mode)
{
	...
	case PANEL_MODE_TVE_NTSC:
		...
		w = TVE_WIDTH_NTSC;
		h = TVE_HEIGHT_NTSC;
		x = (TVE_WIDTH_NTSC - TVE_WIDTH_NTSC) / 2;
		y = (TVE_HEIGHT_NTSC - TVE_HEIGHT_NTSC) / 2;
...
static int jz4760fb_map_smem(struct lcd_cfb_info *cfb)
{
...
#if 1
	/* Medive add  for  lcd flush */
	lcd_frame01 = (unsigned char *)__get_free_pages(GFP_KERNEL,page_shift);
	if (lcd_frame01 == NULL){
		printk("\n Medive printk: get free pages failed!\n");
	}else{
		memset((void *)lcd_frame01,0,PAGE_SIZE << page_shift);
	}
	/* end */
#endif
	...
	//lcd_frame0 = lcd_frame1 + needroom1;
#endif
...
#else
	cfb->fb.fix.smem_start = virt_to_phys((void *)lcd_frame1);
	cfb->fb.fix.smem_len = (PAGE_SIZE << page_shift); /* page_shift/2 ??? */
	cfb->fb.screen_base =
...
static void jz4760fb_descriptor_init( struct jz4760lcd_info * lcd_info )
{
	...
	//medive change
	dma0_desc0->databuf = virt_to_phys((void *)lcd_frame1);
	//dma0_desc0->databuf = virt_to_phys((void *)lcd_frame1);
	...
	//medive change
	dma1_desc0->databuf = virt_to_phys((void *)lcd_frame0);
	//dma1_desc0->databuf = virt_to_phys((void *)lcd_frame0);
		...
		if ( lcd_info->osd.fg_change & FG0_CHANGE_SIZE ) { /* change FG0 size */
			if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { /* output to TV */
				...
				dma0_desc0->page_width = dma0_desc1->page_width
					= fg0_line_size/4;
				dma0_desc1->databuf = virt_to_phys((void *)(lcd_frame1 + fg0_line_size));
				dma0_desc0->next_desc = (unsigned int)virt_to_phys(dma0_desc1);
				dma0_desc1->next_desc = (unsigned int)virt_to_phys(dma0_desc0);
//old
#if 0
		...
		if ( lcd_info->osd.fg_change & FG1_CHANGE_SIZE ) { /* change FG1 size*/
			if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { /* output to TV */
				...
				dma1_desc1->databuf = virt_to_phys((void *)(lcd_frame0 + fg1_line_size));
				dma1_desc0->next_desc = (unsigned int)virt_to_phys(dma1_desc1);
				dma1_desc1->next_desc = (unsigned int)virt_to_phys(dma1_desc0);
#if 0
//old
...
#define PIC_HEIGHT 70
	...
	dma_cache_wback((unsigned int)(lcd_frame0), 320 * 240);
        if(h350_bklight == 0)
...
static int proc_hdmi_read_proc(char *page,char **start,off_t off,int count,int *eof,void *data)
{
	D("Medive printk: read hdmi cur mode is %d\n",hdmi_mode);
	return sprintf(page,"%1u\n",hdmi_mode);
}

static int proc_hdmi_write_proc(struct file *file,const char *buffer,unsigned long count,void *data)
{
	hdmi_mode = simple_strtoul(buffer,0,10);
	D("Medive printk: write hdmi mode is %d\n",hdmi_mode);
	switch (hdmi_mode){
#if defined(CONFIG_JZ4760_HDMI_DISPLAY)  
          case PANEL_MODE_HDMI_480P:
            set_i2s_external_codec();
...
static int proc_tvout_read_proc(char *page,char **start,off_t off,int count,int *eof,void *data)
{
	D("Medive printk: read tvout cur mode is %d\n",tvout_mode);
	return sprintf(page,"%1u\n",tvout_mode);
}

static int proc_tvout_write_proc(struct file *file,const char *buffer,unsigned long count,void *data)
{
	tvout_mode = simple_strtoul(buffer,0,10);
	D("Medive printk: write tvout mode is %d\n",tvout_mode);
	if (tvout_mode){
		jz4760lcd_info_switch_to_TVE(tvout_mode);
		jz4760tve_init(tvout_mode); /* tve controller init */
		udelay(100);
		cpm_start_clock(CGM_TVE);
		jz4760tve_enable_tve();
		REG_TVE_CTRL |= (1 << 21);
	...
	jz4760fb_deep_set_mode(jz4760_lcd_info);
	D("Medive printk: tvout write  end!\n");
	return count;
}

static int proc_bklight_read_proc(char *page,char **start,off_t off,int count,int *eof,void *data)
{
	return sprintf(page,"%1u\n",h350_bklight);
}

static int proc_bklight_write_proc(struct file *file,const char *buffer,unsigned long count,void *data)
{
	h350_bklight = simple_strtoul(buffer,0,10);
	D("\n   lcd  backlight  =  %d\n",h350_bklight);
	if (h350_bklight == 0){
		__lcd_close_backlight();
		mdelay(100);
	...
	mdelay(100);
	__lcd_set_backlight_level(h350_bklight);

	return count;
}
static int proc_lcd_flush_read_proc(char *page,char **start,off_t off,int count,int *eof,void *data)
{
	return sprintf(page,"%1u\n",lcd_flush_flag);
}

static void draw_rgb_rect();
static int proc_lcd_flush_write_proc(struct file *file,const char *buffer,unsigned long count,void *data)
{
	unsigned int cur_flag; 
	if (tvout_mode == 0){
		cur_flag = simple_strtoul(buffer,0,10);
		D("\nMedive printk: lcd flush flag is %d\n",lcd_flush_flag);
		if (cur_flag == 1 && cur_flag != lcd_flush_flag){
			lcd_flush_flag = cur_flag;
			dma0_desc0->databuf = virt_to_phys((void *)lcd_frame1);
		}else if (cur_flag == 0 && cur_flag != lcd_flush_flag){
			lcd_flush_flag = cur_flag;
#if 1
			memcpy(lcd_frame01,lcd_frame0,jz4760_lcd_panel.panel.w * jz4760_lcd_panel.panel.h * jz4760_lcd_panel.osd.fg0.bpp);
			dma_cache_wback((unsigned int)(lcd_frame01),jz4760_lcd_panel.panel.w * jz4760_lcd_panel.panel.h *jz4760_lcd_panel.osd.fg0.bpp );
			dma0_desc0->databuf = virt_to_phys((void *)lcd_frame01);
#else
			memcpy(lcd_frame01,lcd_frame1,jz4760_lcd_panel.panel.w * jz4760_lcd_panel.panel.h * jz4760_lcd_panel.osd.fg0.bpp);
			dma_cache_wback((unsigned int)(lcd_frame01),jz4760_lcd_panel.panel.w * jz4760_lcd_panel.panel.h *jz4760_lcd_panel.osd.fg0.bpp );
			dma0_desc0->databuf = virt_to_phys((void *)lcd_frame01);
...
#if 0
extern unsigned int me_input_reg_count;
...
#if 1
	int i = 0,j = 0;
	unsigned int * fb0 = (unsigned int *)lcd_frame1;
	for (i=0;i<240;i++){
		for (j=0;j<320;j++){
			*fb0++  = 0x00;//0x0000ff00;
		}
...
//	}
	...
	memcpy((unsigned char *)lcd_frame0,(unsigned char *)l009_bootpic,320*240*4);
#else
	int i = 0,j = 0;
	unsigned int * fb0 = (unsigned int *)lcd_frame1;
	for (i=0;i<80;i++){
		for (j=0;j<320;j++){
			*fb0++  = 0x00ff0000;//0x0000ff00;
		}
	}
	for (i=0;i<80;i++){
		for (j=0;j<320;j++){
...
static void spi_write_byte(unsigned char reg,unsigned char data)
{
	int i,j;
...
static void init_rd35_lcd(void)
{
	spi_write_byte(0x01,0x00);
	spi_write_byte(0x02,0x03);//HDNC[1:0]
	spi_write_byte(0x03,0xcc);//c4 or c5
	spi_write_byte(0x04,0x46);
	spi_write_byte(0x05,0x0d);
	spi_write_byte(0x06,0x00);
	spi_write_byte(0x07,0x00);//BIT 3 PAL  INVSL[1:0]FRAD[1:0]
	spi_write_byte(0x08,0x08);
	spi_write_byte(0x09,0x40);            
	spi_write_byte(0x0a,0x88);
	spi_write_byte(0x0b,0x88);
	spi_write_byte(0x0c,0x20);
	spi_write_byte(0x0d,0x20);
	spi_write_byte(0x0e,0x67);
	spi_write_byte(0x0f,0xa4);
	spi_write_byte(0x10,0x04);
	spi_write_byte(0x11,0x24);
	spi_write_byte(0x12,0x24);
	spi_write_byte(0x1E,0x67);	
	spi_write_byte(0x20,0x00);	// WNSEL1[1:0]
}
#endif
...
{
	struct lcd_cfb_info *cfb;
	struct proc_dir_entry *res_tvout,*res_hdmi,*res_bklight,*res_flush,*res_otg;
	...
	D("fb%d: %s frame buffer device, using %dK of video memory\n",
...
//	REG_LCD_PCFG = 0xc4020080;
//	REG_LCD_PCFG = 0xcffa0080;
...
//#if JZ_FB_DEBUG
	...
	/* Medive add hdmi proc*/
	res_hdmi = create_proc_entry("jz/hdmi_me",0,NULL);
	if (res_hdmi)
	{
		//res_hdmi->owner = THIS_MODULE;
		res_hdmi->read_proc = proc_hdmi_read_proc;
		res_hdmi->write_proc = proc_hdmi_write_proc;
		res_hdmi->data = NULL;
	}

	/* Medive add tvout proc*/
	res_tvout = create_proc_entry("jz/tvout_me",0,NULL);
	if (res_tvout)
	{
		//res_tvout->owner = THIS_MODULE;
		res_tvout->read_proc = proc_tvout_read_proc;
		res_tvout->write_proc = proc_tvout_write_proc;
		res_tvout->data = NULL;
	}

	/* Medive add bklight proc*/
	res_bklight = create_proc_entry("jz/lcd_backlight",0,NULL);
	if (res_bklight)
	{
		//res_tvout->owner = THIS_MODULE;
		res_bklight->read_proc = proc_bklight_read_proc;
		res_bklight->write_proc = proc_bklight_write_proc;
		res_bklight->data = NULL;
	}

	/* Medive add lcd-flush proc*/
	res_flush = create_proc_entry("jz/lcd_flush",0,NULL);
	if (res_flush)
	{
		//res_tvout->owner = THIS_MODULE;
		res_flush->read_proc = proc_lcd_flush_read_proc;
		res_flush->write_proc = proc_lcd_flush_write_proc;
		res_flush->data = NULL;
	}

#if 0
	res_otg = create_proc_entry("jz/otg_state", 0, NULL);
	if(res_otg)
	{
		res_otg->read_proc = proc_otg_state_read_proc;
		res_otg->write_proc = proc_otg_state_write_proc;	
		res_otg->data = NULL;
	}
#endif
